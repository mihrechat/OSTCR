...
trans_VOCAB_PATH      = '/root/autodl-tmp/CausalSTGNet/datasets/multi-choice_vqa/TGIF/trans/prior_memory/language/linguistic_vocab.json' 
trans_train_data = '/root/autodl-tmp/CausalSTGNet/datasets/multi-choice_vqa/TGIF/trans/train'
...
def build_linguistic_priors_offline(jsonl_filepath, ln_output_dir):
    ...
    with open(jsonl_filepath, 'r', encoding='utf-8') as f:
        ...
        print(f"===================== Found training dataset: {len(training_dataset_video_ids)} videos ========================")
    ...
    print(f"   Found {len(qtype_counts)} unique Question Types.")
    print(f"   Found {len(role_token_counts)} unique Role-Grounded Tokens.")
    ...
    for qt, count in qtype_counts.items():
        ...
        qtype_expected_embs[qtype_to_idx[qt]] = p_t * emb
...
def build_linguistic_priors_offline_csv_tgif(csv_filepath, ln_output_dir, data_dir):
    ...
    for qt, count in qtype_counts.items():
        ...
        # emb = torch.tensor(text_encoder.encode(qt))
        # qtype_expected_embs[qtype_to_idx[qt]] = p_t * emb
        emb = torch.tensor(text_encoder.encode(qt), dtype=torch.float32)
        qtype_expected_embs[qtype_to_idx[qt]] = p_t * emb
...
def extract_and_save_question_data_csv_tgif():
    ...
    data_dir = "/root/autodl-tmp/CausalSTGNet/datasets/multi-choice_vqa/TGIF/trans/train"
...
def extract_and_save_question_data():
    ...
    data_dir = "/root/autodl-tmp/CausalSTGNet/datasets/open_ended_vqa/MSVD/train"
...
if __name__ == "__main__":
    ...
    ln_output_dir = "/root/autodl-tmp/CausalSTGNet/datasets/multi-choice_vqa/TGIF/trans/prior_memory/language"
