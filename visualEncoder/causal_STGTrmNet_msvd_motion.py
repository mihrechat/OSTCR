...
class STGraphTransformerNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #     nn.GELU(),
        #     nn.Dropout(0.1),
        #     nn.Linear(cfg.model.dim, cfg.model.msvd_vocab_size) 
        # ) 
        self.ez_proj = nn.Linear(cfg.model.text_dim, self.dim)
    ...
    def forward(self, data) -> dict:
        ...
        lengths = lengths.squeeze(-1) # Shape: (B,)
        batch_max_T = lengths.max().item() 
        
        # Slice off the permanent padding! 
        ...
        fused_vector = self.moe_linear_fusion(E_z, E_v, M, qtype_idx) # (B, dim)
