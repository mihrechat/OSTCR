...
class STGraphTransformerNet(nn.Module):
    ...
    def forward(self, data) -> dict:
        ...
        motion_targeted = self.question_motion_crossattentionpooling(Q_deconf, motion_feat, motion_mask) # (B, dim)
        ...
        v_graph_targeted = self.question_crossattention_pooling(Q_deconf, node_feat, batch_idx)
        ...
        causal_targeted = self.causal_fusion(E_z, E_v, M_deconf, Q_deconf)
        ...
        Opt_emb_proj = F.layer_norm(Opt_emb_proj, (self.dim,))
        final_representation = F.layer_norm(final_representation, (self.dim*4,))
        ...
        fused_expanded = fused_proj.unsqueeze(1).expand(-1, num_options, -1) 
