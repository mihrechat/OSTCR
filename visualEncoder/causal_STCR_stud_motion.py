...
class STGraphTransformerNet(nn.Module):
    ...
    def forward(self, data) -> dict:
        ...
        # Suppress the direct shortcut 30% of the time, forcing Causal Path to do the heavy lifting
        # if self.training and torch.rand(1).item() < 0.3:
        #     v_graph_gated = v_graph_gated * 0.2
        #     motion_gated  = motion_gated * 0.2
        ...
        #         print(f"Question proj Norm: {Q_emb_proj.norm(dim=-1).mean():.2f}(std: {Q_emb_proj.std():.3f})")
        ...
        if self.training and torch.rand(1).item() < 0.01:
            ...
            with torch.no_grad():
                ...
                print(f"Motion Norm: {motion_gated.norm(dim=-1).mean():.2f}(std: {motion_gated.std():.3f})")
                print(f"Causal Norm: {causal_gated.norm(dim=-1).mean():.2f}(std: {causal_gated.std():.3f})")
                print(f"Final repre: {final_representation.norm(dim=-1).mean():.2f}(std: {final_representation.std():.3f})")
                print(f"option Norm: {Opt_emb_proj.norm(dim=-1).mean():.2f}(std: {Opt_emb_proj.std():.3f})")
