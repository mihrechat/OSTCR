...
class RobustTripletMediator(nn.Module):
    ...
    """
    def __init__(self, dim: int, mediator_dim: int, attn_dim: int, dropout: float = 0.1):
        super().__init__()
        self.W_q = nn.Sequential(
            nn.Linear(mediator_dim, attn_dim),
...
class STGraphTransformerNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # ── Stage 3: Question Encoding
        self.question_encoder = nn.Sequential(
            nn.Linear(cfg.model.text_dim, self.dim),
        ...
        # ── Stage 6: Motion Processing
        self.motion_proj = nn.Sequential(
            nn.Linear(cfg.model.motion_dim, self.dim),
            nn.LayerNorm(self.dim),
            nn.GELU(),
            nn.Dropout(self.dropout)
        )
    ...
    def forward(self, data) -> dict:
        ...
        lengths = data["num_temporal_slots"].squeeze(-1)
