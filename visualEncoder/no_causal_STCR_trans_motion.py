...
class STGraphTransformerNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.film_hidden = nn.Linear(self.dim, self.dim * 2) # *2 for gamma (scale) and beta (shift)
        self.visual_to_text_proj = nn.Linear(self.dim, self.dim, bias=False) #
    ...
    def forward(self, data) -> dict:
        ...
        # ── NEW: Question-Adaptive FiLM Projection ──
        # 1. Use Q_emb to generate scale (gamma) and shift (beta) for EACH node
        film_params = self.film_hidden(Q_emb) # (B, node_dim * 2)
        gamma, beta = film_params.chunk(2, dim=-1) # Both are (B, node_dim)
        
        # 2. Expand to nodes and apply: Node = Node * Scale + Shift
        gamma_expanded = gamma[batch_idx] # (N, node_dim)
        beta_expanded = beta[batch_idx]   # (N, node_dim)
        
        modulated_nodes = (final_node_repr * gamma_expanded) + beta_expanded
        
        # final_node_repr = q_aware_nodes + motion_expanded + causal_expanded
        
        node_logits_proj = self.visual_to_text_proj(modulated_nodes) 
