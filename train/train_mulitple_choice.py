...
def train_one_epoch(
    ...
    }, current_scheduler, global_step + n_batches
...
if __name__ == "__main__":
    ...
    train_dataset = TGIFDatasetMotion(cfg, split="train")
    
    all_video_ids = list(set([x["video_id"] for x in train_dataset.qa_pairs]))
    ...
    split_info = torch.load("/root/autodl-tmp/tgif_split.pt")

    train_dataset = TGIFDatasetMotion(
        cfg,
        split="train",
        allowed_video_ids=set(split_info["train_video_ids"])
    )

    val_dataset = TGIFDatasetMotion(
        cfg,
        split="train",
        allowed_video_ids=set(split_info["val_video_ids"])
    )

    # train_dataset = TGIFDatasetMotion(
    #     cfg,
    #     split="train",
    #     allowed_video_ids=set(train_video_ids)
    # )

    # val_dataset = TGIFDatasetMotion(
    #     cfg,
    #     split="train",
    #     allowed_video_ids=set(val_video_ids)
    # )
    model   = STGraphTransformerNet(cfg)
    train(model, train_dataset, val_dataset, cfg)
